...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        A1, cache_A1 = affine_relu_forward(X, W1, b1)
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(1, self.num_layers):
          self.params['W' + str(i)] = std * np.random.randn(hidden_dim_prev, hidden_dims[i-1])
        ...
        i+=1
        self.params['W' + str(i)] = std * np.random.randn(hidden_dim_prev, num_classes)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
          ...
          if self.normalization == "batchnorm":
            ...
            A, cache = affine_batchnorm_relu_forward(A, W, b, gamma, beta, self.bn_params[i-1])

          elif self.normalization == "layernorm":
            gamma, beta = self.params['gamma' + str(i)], self.params['beta' + str(i)]
            A, cache = affine_layernorm_relu_forward(A, W, b, gamma, beta, self.bn_params[i-1])
        ...
        dA, dW, db = affine_backward(dZ, cache)
